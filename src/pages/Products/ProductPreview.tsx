...
export default function ProductPreviewCustomer() {
    ...
    const additionalOptions = Object.entries(product.priceConfiguration || {}).filter(
        ([, cfg]) => cfg.priceType === "additional"
    );

    // Get numeric price for selected base
    const basePrice = selectedBase
        ? Number(product.priceConfiguration[selectedBase]?.availableOptions.price)
        : 0;
    ...
    const addonsTotal = selectedAddons.reduce((sum, addonKey) => {
        ...
        const optionPrice =
            product.priceConfiguration[groupKey]?.availableOptions?.[optionName] || 0;
                        ...
                        <>
                            ...
                            <Space direction="vertical">
                                ...
                                {additionalOptions.length > 0 && (
                                    <>
                                        <Space direction="vertical">
                                            {additionalOptions.map(([key, cfg]) =>
                                                Object.entries(cfg.availableOptions || {}).map(([optionName, price]) => {
                                                        ...
                                                        <Checkbox
                                                            ...
                                                            onChange={(e) => {
                                                                if (e.target.checked) {
                                                                    ...
                                                                    setSelectedAddons(
                                                                        selectedAddons.filter((k) => k !== addonKey)
                                                                    );
                                                                }
                                                            }}
                                                        >
