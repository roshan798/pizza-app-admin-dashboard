...
export default function CategoryDetails() {
    ...
    return (
        <Card
            title={<Title level={3} style={{ margin: 0 }}>{category.name}</Title>}
            extra={
                <Space size="small">
                    {/* Add action buttons here later: Edit, Duplicate, Archive */}
                </Space>
            }
            styles={{
        ...
        >
            ...
            <Descriptions
                ...
                colon={false}
                items={[
                    { key: "created", label: "Created", children: createdAt },
                    { key: "updated", label: "Updated", children: updatedAt },
            ...
            {category.priceConfiguration &&
                ...
                <Row gutter={[16, 16]}>
                    {Object.entries(category.priceConfiguration).map(([key, cfg]) => (
                        <Col xs={24} sm={12} md={8} key={key}>
                            <Card size="small" title={key} variant="outlined">
                                <Space direction="vertical" size={8} style={{ width: "100%" }}>
                                    <div>
                                        <Text strong>Price Type: </Text>
                                        <Tag color={cfg.priceType === "base" ? "green" : "blue"}>
                                            {cfg.priceType}
                                        </Tag>
                                    </div>
                                    <div>
                                        ...
                                        <div style={{ marginTop: 8 }}>
                                            {cfg.availableOptions?.length ? (
                                                cfg.availableOptions.map((opt) => (
                                                    <Tag key={opt} style={{ marginBottom: 8 }}>
                                                        {opt}
                                                    </Tag>
                                                ))
                                            ) : (
                ...
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No price configurations" />
            )}
            ...
            {category.attributes?.length ? (
                <Row gutter={[16, 16]}>
                    {category.attributes.map((attr, index) => (
                        <Col xs={24} sm={12} md={8} key={`${attr.name}-${index}`}>
                            <Card size="small" title={attr.name} bordered>
                                <Space direction="vertical" size={8} style={{ width: "100%" }}>
                                    <div>
                                        <Text strong>Widget Type: </Text>
                                        <Tag color="purple">{attr.widgetType}</Tag>
                                    </div>
                                    <div>
                                        <Text strong>Default Value: </Text>
                                        {attr.defaultValue ? (
                                            <Tag color="cyan">{attr.defaultValue}</Tag>
                                        ) : (
                                            <Text type="secondary">None</Text>
                                        )}
                                    </div>
                ...
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No attributes" />
            )}
        </Card>
    );
}
