...
const OrderSummaryTotalsCard: React.FC<OrderSummaryTotalsCardProps> = ({ amounts, couponCode }) => {
    return (
        <Card
            title={
                <Flex align="center" gap="small">
                    <DollarCircleOutlined style={{ fontSize: '20px', color: '#52c41a' }} />
            ...
            <Space direction="vertical" style={{ width: '100%' }} size="small">
                ...
                {amounts.discount > 0 && (
                    <Flex justify="space-between">
                        ...
                        <Text strong type="success">- ₹{amounts.discount.toFixed(2)}</Text>
                    </Flex>
                )}
                <Divider style={{ margin: '12px 0' }} />
                <Flex justify="space-between" align="center">
                    <Title level={4} style={{ margin: 0 }}>Total Amount</Title>
                    <Title level={4} style={{ margin: 0 }}>₹{amounts.grandTotal.toFixed(2)}</Title>
                </Flex>
                {couponCode && (
