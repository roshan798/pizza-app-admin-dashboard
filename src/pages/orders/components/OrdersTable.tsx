...
import { Table, Button, Space, Flex, Tag, Badge, Empty, Typography, Card } from 'antd';
import {
    EnvironmentOutlined,
    PhoneOutlined,
    ClockCircleOutlined,
...
const OrdersTable: React.FC<OrdersTableProps> = ({
    ...
    const getStatusConfig = (status: Order['orderStatus']): StatusConfig => {
        const configMap: Record<Order['orderStatus'], StatusConfig> = {
            ...
            'verified': { color: 'processing', icon: <CheckCircleOutlined />, text: 'Verified' },
            'confirmed': { color: 'processing', icon: <CheckCircleOutlined />, text: 'Confirmed' },
            'preparing': { color: 'warning', icon: <InboxOutlined />, text: 'Preparing' },
            'out-for-delivery': { color: 'warning', icon: <EnvironmentOutlined />, text: 'Out for Delivery' },
            'delivered': { color: 'success', icon: <CheckCircleOutlined />, text: 'Delivered' },
            'cancelled': { color: 'error', icon: <CloseCircleOutlined />, text: 'Cancelled' }, // Ensure 'error' is a valid Badge status
        };
    ...
    type PaymentConfig = {
        color: 'green' | 'volcano' | 'geekblue' | 'orange' | 'default' | 'success' | 'error' | 'warning'; // Tag colors are more flexible
        text: string;
    };

    const getPaymentConfig = (status: Order['paymentStatus']): PaymentConfig => {
    ...
    const columns: ColumnsType<Order> = [
        { 
            ...
            render: (id: string, record: Order) => (
                <Flex vertical gap={4}>
                    <Text strong ellipsis style={{ width: 80 }}># {id.slice(-8)}</Text>
            ...
            render: (_: unknown, record: Order) => (
                <Space direction="vertical" size="small">
                    <Flex gap={4} align="center">
                        ...
                        <Text ellipsis style={{ maxWidth: 120 }} title={record.address}>
        ...
        },
        {
            ...
            key: 'amount',
            width: 120,
            sorter: (a: Order, b: Order) => a.amounts.grandTotal - b.amounts.grandTotal,
            render: (_: unknown, record: Order) => (
                <Flex vertical align="start" gap={4}>
                    <Text strong>₹{record.amounts.grandTotal.toFixed(2)}</Text>
                    {record.couponCode && (
                        <Tag color="purple" style={{ fontSize: 11 }}>COUPON</Tag>
                    )}
                </Flex>
            ),
        },
